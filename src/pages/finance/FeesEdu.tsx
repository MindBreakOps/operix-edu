...
import { Plus, Wallet, Trash2, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
export default function FeesEdu() {
  ...
  const fetchData = async () => {
	...
	// Strict Portal Isolation for Fees
	...
	// Restrict dropdown students to active portal
  ...
  return (
	<PageShell
	  ...
	  subtitle="إدارة المستحقات الدراسية لطلاب هذه البوابة"
	  ...
	  {isLoading ? (
		...
		<table style={{ width: '100%', borderCollapse: 'collapse' }}>
		  ...
		  <tbody>
			  ...
			  <tr key={f.id}>
				<td style={{ ...styles.td, fontWeight: 800, color: 'var(--color-navy)' }}>
				  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
					<Wallet size={16} color="var(--color-royal)" />
			...
			{fees.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--color-text-muted)', fontWeight: 800 }}>لا توجد سجلات مالية مسجلة.</td></tr>}
		  </tbody>
		</table>
	  )}
	  ...
	  {isModalOpen && (
		<div className="no-print" style={styles.modalOverlay}>
		  <div style={{ backgroundColor: 'var(--color-white)', padding: '32px', borderRadius: '16px', width: '100%', maxWidth: '600px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
			<h2 style={{ margin: '0 0 24px 0', color: 'var(--color-navy)', fontSize: '1.4rem', fontWeight: 900 }}>إنشاء فاتورة جديدة</h2>
			<form onSubmit={handleSave}>
			  ...
			  <div style={styles.inputGroup}>
				<label style={styles.label}>وصف الرسوم</label>
			  ...
			  <div style={{ display: 'flex', gap: '16px' }}>
				...
				<div style={{ ...styles.inputGroup, flex: 1 }}>
				  <label style={styles.label}>حالة الدفع الأولية</label>
