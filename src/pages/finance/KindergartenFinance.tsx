...
import { Plus, Baby, Trash2, CheckCircle, Clock, AlertCircle } from 'lucide-react';
import { PageShell } from '../../components/layout/PageShell';

export default function KindergartenFinance() {
  ...
  const [formData, setFormData] = useState({ 
	...
	amount: 0, 
	currency: 'SAR', 
	description: 'رسوم التسجيل الأساسية', 
	status: 'غير مدفوع', 
	due_date: new Date().toISOString().split('T')[0] 
  });

  const kgFeeTypes = [
	'رسوم التسجيل الأساسية',
	'رسوم الزي المدرسي والكتب',
	'اشتراك الوجبات الغذائية',
	'الرعاية الممتدة (الفترة المسائية)',
	'رسوم الأنشطة والرحلات'
  ];

  const fetchData = async () => {
	// We only fetch data if the current portal is explicitly kindergarten
	// If a user navigates here from another portal, it will restrict the view.
	const targetPortal = 'kindergarten';
	if (!workspace) return;
	setIsLoading(true);
	...
	const { data: fData } = await supabase
	  ...
	  .eq('students_edu.portal_type', targetPortal)
  ...
  const handleSave = async (e: React.FormEvent) => {
	...
	const payload = {
	  ...
	  portal_type: 'kindergarten', // Hardcoded isolation for this specific module
	  student_id: formData.student_id,
	  amount: formData.amount,
	  currency: formData.currency,
	  description: `رياض أطفال: ${formData.description}`,
	...
	if (!error) {
	  ...
	  fetchData();
	  setFormData({ student_id: '', amount: 0, currency: 'SAR', description: 'رسوم التسجيل الأساسية', status: 'غير مدفوع', due_date: new Date().toISOString().split('T')[0] });
	} else {
  ...
  if (portalType !== 'kindergarten') {
	return (
	  <div style={{ padding: '40px', textAlign: 'center', fontWeight: 900, color: 'var(--color-danger)' }}>
		هذه الصفحة مخصصة لبوابة رياض الأطفال فقط.
	  </div>
	);
  }
  ...
  return (
	<PageShell
	  title="مالية رياض الأطفال"
	  subtitle="إدارة الرسوم، الوجبات، والرعاية الممتدة لطلاب الروضة والتمهيدي"
	  onPrint={() => window.print()}
	  actionButton={
		<button style={styles.btnPrimary} onClick={() => setIsModalOpen(true)}>
		  <Plus size={18} /> إنشاء فاتورة طفل
		</button>
	  }
	>
	  {isLoading ? (
		<div style={{ padding: '40px', textAlign: 'center', color: 'var(--color-text-muted)', fontWeight: 800 }}>جاري جلب السجلات المالية...</div>
	  ) : (
		<table style={{ width: '100%', borderCollapse: 'collapse' }}>
		  <thead>
			<tr>
			  <th style={styles.th}>الطفل</th>
			  <th style={styles.th}>بند الرسوم</th>
		  ...
		  <tbody>
			  ...
			  <tr key={f.id}>
				<td style={{ ...styles.td, fontWeight: 800, color: 'var(--color-navy)' }}>
				  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
					<Baby size={16} color="var(--color-royal)" />
			...
			{fees.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--color-text-muted)', fontWeight: 800 }}>لا توجد سجلات مالية لرياض الأطفال.</td></tr>}
		  </tbody>
		</table>
	  )}
	  ...
	  {isModalOpen && (
		<div className="no-print" style={styles.modalOverlay}>
		  <div style={{ backgroundColor: 'var(--color-white)', padding: '32px', borderRadius: '16px', width: '100%', maxWidth: '600px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
			<h2 style={{ margin: '0 0 24px 0', color: 'var(--color-navy)', fontSize: '1.4rem', fontWeight: 900 }}>إنشاء فاتورة رياض أطفال</h2>
			<form onSubmit={handleSave}>
			  
			  <div style={styles.inputGroup}>
				<label style={styles.label}>الطفل المسجل</label>
			  ...
			  <div style={styles.inputGroup}>
				<label style={styles.label}>بند الرسوم</label>
				<select required style={styles.input} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})}>
				  {kgFeeTypes.map(type => <option key={type} value={type}>{type}</option>)}
				  <option value="رسوم مخصصة أخرى">رسوم مخصصة أخرى</option>
				</select>
			  </div>
